...
def image_data_to_custom_vision_create_entry(image_data, map_tag_name_to_id):
    image_file = open(image_data.image_path, mode="rb")
    ...
    create_entry = ImageFileCreateEntry(name=Path(image_data.image_path).name, contents=image_file.read(), regions=custom_vision_regions)
    image_file.close()
    return create_entry

def LocalData(strategy, base_directory):
    """
    ...
    class YOLOStrategy:
        def __init__(self, base_directory):
            ...
            # Get LocalData representation for all eligible image representations.
            for data_filename in data_filenames:
                data_path = os.path.join(base_directory, data_filename)
                image_path = os.path.join(base_directory, data_filename.replace(".txt", ".jpg"))
                image_data = ImageData(data_path, image_path)
                image_data.populate_regions_from_yolo_data(class_names)

                local_data.append(image_data)
            self.image_data = local_data

            # Load tags from disk.
            self.tags = class_names
    
    if strategy == "YOLOStrategy":
        return YOLOStrategy(base_directory)
    else:
        raise Exception(f"No strategy \"{strategy}\" is defined!")
...
def main():
    ...
    def upload_local_data(local_data):
        ...
        for i, data in enumerate(local_data.image_data):
            custom_vision_data = image_data_to_custom_vision_create_entry(data, map_tag_name_to_id)  # Loads image and backing data into memory
    ...
    local_data = LocalData("YOLOStrategy", input_image_directory)
